...
def execute_op(context)
  ...
  when 5
    jit(context, param_modes)
  when 6
    jif(context, param_modes)
  when 7
    less_than(context, param_modes)
  when 8
    equals(context, param_modes)
  when 99
    puts "Program Halted"
    exit
  else
...
def less_than(context, param_modes)
  address_or_value1 = context[$address + 1].to_i
  address_or_value2 = context[$address + 2].to_i
  param1 = param_modes[0] == 0 ? context[address_or_value1].to_i : address_or_value1
  param2 = param_modes[1] == 0 ? context[address_or_value2].to_i : address_or_value2
  ...
  if param1 < param2
    context[output_address] = 1.to_s
  else
    context[output_address] = 0.to_s
  end
  $address += 4
end

def equals(context, param_modes)
  address_or_value1 = context[$address + 1].to_i
  address_or_value2 = context[$address + 2].to_i
  param1 = param_modes[0] == 0 ? context[address_or_value1].to_i : address_or_value1
  param2 = param_modes[1] == 0 ? context[address_or_value2].to_i : address_or_value2
